...
def train_lut_single(model, direction_feature, x_view, GT, edge_mask, image_name, optimizer,
    ...
    for idx, step in enumerate(pbar):
        y_up, sum16 = model.E_infer(direction_feature.detach(), x_view.detach())

        # 加权 L2 Loss
        loss_map = L2loss(y_up, GT[:, :3, 1:-1, 1:-1])
        L2_loss = torch.mean(loss_map * edge_mask) * 100

        ssim_map = 1-ssim(y_up, GT[:, :3, 1:-1, 1:-1], window_size=7, size_average=False)
        ssim_loss = torch.mean(ssim_map * edge_mask)
    ...
    torch.save({
        ...
        'step': step
    }, os.path.join(save_dir, f'E_ckp_{step}.pth'))
...
def Super_Resolution(model, data_list, save_id, Scale=2, num_epochs=20):
    ...
    for epoch in range(num_epochs):
        ...
        for (LR_color, LR_depth, LR_mv, HR_color, image_name) in data_list:
            ...
            train_lut_single(model, Depth_direction_feature, img, GT, edge_mask, image_name, optimizer)
...
if __name__ == '__main__':
    ...
    try:
        resume_checkpoint = torch.load('E_ckp_4999.pth')
